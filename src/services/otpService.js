...
const otpService = {
  ...
  verify: async ({ otp, email, transaction }) => {
    try {
      ...
      if (!isMatch) {
        if (lastOtp.attempts + 1 >= 3) {
          throw new customError(
            ...
            "OTP entry count exceeded",
            { EC: 102 }
          );
        }
        await Otp.update(
          { attempts: lastOtp.attempts + 1 },
          { where: { id: lastOtp.id }, transaction }
        );
        throw new customError(StatusCodes.BAD_REQUEST, "Incorrect OTP code", {
          EC: 103,
        });
      }
    } catch (error) {
